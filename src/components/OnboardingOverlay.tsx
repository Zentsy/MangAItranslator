...
const OnboardingOverlay: React.FC = () => {
  ...
  const needsApiKey = !hasFinishedOnboarding && translationEngine === "gemini" && !apiKey;
  ...
  useEffect(() => {
    ...
    const anchor = document.querySelector<HTMLElement>('[data-gemini-key-anchor="true"]');
    if (!anchor) {
      setPopoverPosition(null);
      return;
    }
    ...
    const updatePosition = () => {
      ...
      const left = Math.min(
        Math.max(rect.left + rect.width / 2 - PANEL_WIDTH / 2, VIEWPORT_PADDING),
        window.innerWidth - PANEL_WIDTH - VIEWPORT_PADDING
      );
      const arrowLeft = Math.min(
        Math.max(rect.left + rect.width / 2 - left, ARROW_PADDING),
        PANEL_WIDTH - ARROW_PADDING
      );
          ...
          popoverPosition ? (
            ...
            >
              <div className="relative rounded-[2.5rem] border border-app-border bg-app-surface p-6 text-app-text-primary shadow-[0_20px_50px_rgba(0,0,0,0.3)]">
                <div
                  className="absolute -top-2 h-4 w-4 -translate-x-1/2 rotate-45 border-l border-t border-app-border bg-app-surface"
